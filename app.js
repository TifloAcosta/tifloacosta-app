(() => {
  ...
  const copy = {
    es: {
      ...
      moreOptions: { videos: 'Vídeos', book: 'Libro', community: 'Comunidad', social: 'Redes sociales', library: 'Biblioteca completa', settings: 'Ajustes de visualización y accesibilidad', install: 'Instalar la app', notifications: 'Notificaciones' },
      ...
      panels: {
        ...
        install: '<h3>Instalar la app</h3><p>Cuando esta PWA esté publicada mediante HTTPS, podrá instalarse desde Safari, Chrome o Edge según la plataforma. Esta copia local sirve únicamente para probar la navegación.</p>',
      ...
      }, footer: 'TifloAcosta App · Versión 0.3 de prueba accesible.'
    },
    en: {
      ...
      moreOptions: { videos: 'Videos', book: 'Book', community: 'Community', social: 'Social media', library: 'Full library', settings: 'Display and accessibility settings', install: 'Install the app', notifications: 'Notifications' },
      ...
      panels: {
        ...
        install: '<h3>Install the app</h3><p>Once this PWA is published over HTTPS, it can be installed from Safari, Chrome, or Edge depending on the platform. This local copy is only for navigation testing.</p>',
      ...
      }, footer: 'TifloAcosta App · Accessible test version 0.3.'
    }
  };
  ...
  function renderSettingsPanel() {
    ...
    form.append(textFs,themeFs,spacingFs,boldFs,actions,status);els.morePanel.append(h3,intro,form);
  }

  function renderMoreOptions(){const c=copy[lang],values=['','videos','book','community','social','library','settings','install','notifications'];els.moreSelect.innerHTML='';values.forEach(v=>option(els.moreSelect,v,v?c.moreOptions[v]:c.morePlaceholder));}
  function showMore(value){if(!value){els.morePanel.hidden=true;els.morePanel.innerHTML='';return;}els.morePanel.hidden=false;if(value==='settings'){renderSettingsPanel();return;}els.morePanel.innerHTML=copy[lang].panels[value];}

  function applyLanguage(){const c=copy[lang];document.documentElement.lang=lang;localStorage.setItem('tifloLang',lang);els.langEs.setAttribute('aria-pressed',String(lang==='es'));els.langEn.setAttribute('aria-pressed',String(lang==='en'));els.intro.textContent=c.intro;els.searchHeading.textContent=c.searchHeading;els.search.previousElementSibling.textContent=c.searchLabel;els.search.placeholder=c.placeholder;els.searchButton.textContent=c.searchButton;els.newsHeading.textContent=c.news;els.exploreHeading.textContent=c.explore;els.category.previousElementSibling.textContent=c.categoryLabel;els.favoritesButton.textContent=c.favorites;els.clearResults.textContent=c.clear;els.moreHeading.textContent=c.more;els.moreSelect.previousElementSibling.textContent=c.moreLabel;els.footer.textContent=c.footer;renderCategories();renderMoreOptions();renderNews();clearResults();showMore('');}
