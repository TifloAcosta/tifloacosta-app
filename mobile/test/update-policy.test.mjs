...
test('falls back to immediate only when flexible is unavailable', () => {
  assert.equal(chooseUpdateMode({ available: true, priority: 2, flexibleAllowed: false, immediateAllowed: true }), 'immediate');
});

test('unavailable update produces none', () => {
  assert.equal(chooseUpdateMode({ available: false }), 'none');
  assert.equal(chooseUpdateMode({ available: true, flexibleAllowed: false, immediateAllowed: false }), 'none');
});
