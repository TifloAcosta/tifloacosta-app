...
test('service worker treats changing catalogs and scripts as live content', async () => {
  ...
  assert.match(worker, /'\.\/podcast\.json'/);
  ...
  assert.match(worker, /url\.pathname\.endsWith\('\/podcast\.json'\)/);
  assert.match(worker, /tifloacosta-app-v2-4-search-podcast-categories/);
});
