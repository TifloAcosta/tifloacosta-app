...
test('homepage loads shared app logic and reserves the lazy book-cover dimensions', async () => {
  ...
  assert.match(html, /<script src="app-core\.js\?v=1\.4"><\/script>\s*<script src="actualidad-core\.js\?v=[^"]+"><\/script>\s*<script src="app\.js\?v=[^"]+"><\/script>/);
  assert.match(html, /<img id="book-cover"[^>]*loading="lazy"[^>]*width="1600"[^>]*height="2560">/);
  assert.match(worker, /'\.\/app-core\.js\?v=1\.4'/);
});
